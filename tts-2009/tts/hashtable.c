...
void clearHashTable (HashTablePtr hashTable)
{
	...
	freeJustHashSlots (hashTable);
	//printHashTable (hashTable);
}

void freeJustHashSlots (HashTablePtr hashTable)
{
	int i;
	...
	hashTable->currentNumOfSlots = 3;
	hashTable->previousNumOfSlots = 1;
	hashTable->currentSlots = (HashLinkPtr *)checkMalloc (hashTable->currentNumOfSlots*sizeof(HashLinkPtr),"Hash slots");
	hashTable->previousSlots = (HashLinkPtr *)checkMalloc (hashTable->previousNumOfSlots*sizeof(HashLinkPtr),"Hash slots");

	for (i=0;i<hashTable->currentNumOfSlots;i++)
		hashTable->currentSlots[i] = NULL;
	for (i=0;i<hashTable->previousNumOfSlots;i++)
		hashTable->previousSlots[i] = NULL;
...
int oneRemove (unsigned long hashValue, HashTablePtr hashTable, HashLinkPtr *slots, int numOfSlots, void *k)
{
	HashLinkPtr link, previous;
	previous = NULL;
	...
	{
		if (previous == NULL)
			slots[hashValue % numOfSlots] = link->next;
		else
...
void hashTableRemove (HashTablePtr hashTable, void *k)
{
	unsigned long hashValue;
	...
	if (oneRemove (hashValue, hashTable, hashTable->currentSlots, hashTable->currentNumOfSlots, k))
		hashTable->currentNumOfKeys--;
	if (oneRemove (hashValue, hashTable, hashTable->previousSlots, hashTable->previousNumOfSlots, k))
		hashTable->previousNumOfKeys--;
	//hashTable->currentRehashSlot = 0;
	//hashTable->currentRehashLink = NULL;
