...
tmp=$1											# seconds parameter is temporary directory
shift

# binary of the used SAT solver
satsolver=./pcasso-port.sh  			# name of the binary (if not in this directory, give relative path as well)

# default parameters for preprocessor
cp3params=" -enabled_cp3 -cp3_stats -up -subsimp -all_strength_res=3 -bva -cp3_bva_limit=120000 -bve -bve_red_lits=1 -no-bve_BCElim -unhide -no-cp3_uhdUHLE -cp3_uhdIters=5 -dense"
