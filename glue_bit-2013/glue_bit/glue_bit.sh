...
if [ "x$1" = "x" ]; then
  echo "USAGE: glue_bit.sh <input CNF> TMPDIR"
  exit 1
fi
...
TMP=$TMPDIR/glue_bit_$$ #set this to the location of temporary files
SE=$mypath/SatELite_release           #set this to the executable of SatELite
RS=$mypath/glue_bit_static              #set this to the executable of RSat
INPUT=$1;
shift 
echo "c"
...
if [ $X == 0 ]; then
  ...
  #SatElite died, glue_bit must take care of the rest
    $RS $INPUT #but we must force glue_bit to print out result here!!!
  X=$?
elif [ $X == 12 ]; then
