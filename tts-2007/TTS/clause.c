...
#ifdef PROOF
	result->realClauseNum = c->realClauseNum;
#endif
...
#ifdef PROOF
	result->realClauseNum = clause->realClauseNum;
#endif
...

{
	...
	{
		stillFalse = clause->positive[i] != assignment->values[globalTransform[clause->variables[i]]];
