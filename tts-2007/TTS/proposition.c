...
#ifdef PROOF
				result->falsifyingClauseNum = p->clauses[c]->realClauseNum;
#endif
...
#ifdef PROOF
	ProofClausePtr *unitClauseProofPtr;
#endif

#ifdef PROOF    
	unitClauseProofPtr = (ProofClausePtr *) checkMalloc ( (1+maxVariable)*sizeof (ProofClausePtr),"Unit clause proofs");
#endif    
...
*/
			if (clause->numOfVariables == 1)
			{
				...
				if (used[v1])
				{
					...
					{
#ifdef PROOF
						unitClauseProofPtr[v1] = clause->realClauseNum;
#endif
...
*/
			...
			while (v1 < clause->numOfVariables && clauseNeeded)
			{
				...
				{
#ifdef PROOF
					clause->realClauseNum = resolveProofClauses (3, clause->positive[v1], vv,
						unitClauseProofPtr[vv],clause->realClauseNum);
#endif
...
int elementLinkage (int v, SetPtr set, int **linkage)
{
	int result=0, i;
	for (i=0;i<set->numOfElements;i++)
		result += linkage[v][(int)(set->elements[i])];
	return result;
}

int overallCost (SetPtr set, int **linkage, PropositionPtr p)
{
	int i,j,result,v1,c, max,found;
	ClausePtr clause;
	...
	for (c=0; c<p->numOfClauses;c++)
	{
		...
		for (i=0;i<clause->numOfVariables;i++)
		{
			...
			found = false;
			for (j=0;j<set->numOfElements&&!found;j++)
				found = v1 == (int)(set->elements[j]);
			if (j+1 > max)
				max = j+1;
		}
		result += max;
...
int overallCost1 (SetPtr set, int **linkage, PropositionPtr p)
{
	int i,j,l,result,v1,v2;
	
	result = 0;
	for (i=0; i<set->numOfElements;i++)
		for (j=i+1; j<set->numOfElements;j++)
		{
			v1 = (int) (set->elements[i]);
			v2 = (int) (set->elements[j]);
			l = j-i;
			result += l * linkage[v1][v2];
		}
	return result;     
}
...
int elementLinkageWithout (int v, SetPtr set, int **linkage, int ignore)
{
	int result=0, i;
	for (i=0;i<set->numOfElements;i++)
		if (i != ignore)
...
int setLinkage (SetPtr s1, SetPtr s2, int **linkage)
{
	int result = 0, i;
	if (s2 != NULL)
	{
		for (i=0;i<s1->numOfElements;i++)
			result += elementLinkage ((int)(s1->elements[i]), s2, linkage);
	}
	return result;
}

void findBestSwapper (SetPtr thisSet, SetPtr otherSet, int **linkage, int *result, int *resultCost, int ignore)
{
	int v,i, thisCost;
	
	i = 0;
	v = (int)(thisSet->elements[i]);
	*resultCost = elementLinkageWithout (v, otherSet, linkage, ignore) - elementLinkageWithout (v, thisSet, linkage, i);
	*result = i;
	for (i=1;i<thisSet->numOfElements;i++)
	{
		v = (int)(thisSet->elements[i]);
		thisCost = elementLinkageWithout (v, otherSet, linkage, ignore) - elementLinkageWithout(v, thisSet, linkage, i);
		if (thisCost > *resultCost)
		{
			*resultCost = thisCost;
			*result = i;
		}
...
void sortVarSet (SetPtr set, int **linkage, int numOfVariables, SetPtr sibling, int siblingToRight)
{
	SetPtr leftSet,rightSet;
	int v1,v2, maxLeftElements, maxRightElements,
		leftLinkage, rightLinkage;
	int swap, temp;
	
	if (set->numOfElements < 2)
		return;
		
	maxLeftElements = set->numOfElements/2;
	maxRightElements = set->numOfElements - maxLeftElements;

	leftSet = newSet();
	rightSet = newSet();
	
	for (v1=0; v1<set->numOfElements; v1++)
	{
		v2 = (int)(set->elements[v1]);
		leftLinkage = elementLinkage (v2, leftSet, linkage);
		rightLinkage = elementLinkage (v2, rightSet, linkage);
		if (leftSet->numOfElements == maxLeftElements)
		{
			addToSetNoCheck (rightSet, (void *)v2);
		}
		else if (rightSet->numOfElements == maxRightElements)
		{
			addToSetNoCheck (leftSet, (void *)v2);
		}
		else if (leftLinkage > rightLinkage)
		{
			addToSetNoCheck (leftSet, (void *)v2);
		}
		else if (leftLinkage < rightLinkage)
		...
		else if (leftSet->numOfElements < rightSet->numOfElements)
		{
			addToSetNoCheck (leftSet, (void *)v2);
		}
		else      
	...
	while (swap)
	{
		for (v1=v1+1;v1<leftSet->numOfElements&&
			elementLinkage ((int)(leftSet->elements[v1]), leftSet,linkage) >
			elementLinkage ((int)(leftSet->elements[v1]), rightSet,linkage); v1++);
		for (v2=v2+1;v2<rightSet->numOfElements&&
			elementLinkage ((int)(rightSet->elements[v2]), rightSet,linkage) >
			elementLinkage ((int)(rightSet->elements[v2]), leftSet,linkage); v2++);
			
		if (v1 < leftSet->numOfElements && v2 < rightSet->numOfElements)
		{
			temp = (int)(leftSet->elements[v1]);
			leftSet->elements[v1] = rightSet->elements[v2];
			rightSet->elements[v2] = (void *)(temp);
		}
		else
			swap = false;
	}
	
	sortVarSet (leftSet, linkage, numOfVariables, rightSet, true);
	sortVarSet (rightSet, linkage, numOfVariables, leftSet, false);
	
	if (setLinkage (leftSet, sibling, linkage) > setLinkage (rightSet, sibling, linkage) && siblingToRight)
	{
		for (v1=0;v1<rightSet->numOfElements;v1++)
			set->elements[v1] = rightSet->elements[v1];
		for (v1=0;v1<leftSet->numOfElements;v1++)
			set->elements[rightSet->numOfElements + v1] = leftSet->elements[v1];
	}
	else
	{
		for (v1=0;v1<leftSet->numOfElements;v1++)
			set->elements[v1] = leftSet->elements[v1];
		for (v1=0;v1<rightSet->numOfElements;v1++)
			set->elements[leftSet->numOfElements + v1] = rightSet->elements[v1];
	}
	freeSet (leftSet);
	freeSet (rightSet);
}
