...
int *globalMapVariable, *globalUnMapVariable, *singletonUsage, *isSingleton, *globalUsed, *globalPermute,
...
AssignmentPtr ttsCore (PropositionPtr p)
{
	...
	startSet = newSet();
	addToSet (startSet, ternaryTree);
	startFalse = newSet();
	startNewUnitVars = newSet();
	
	if (!buildAndTryTernary (answer,0,startSet,startFalse))
		answer = NULL;
...
SubProblemPtr newSubProblem(int maxVariable)
{
	...
	result->permute = (int *) checkMalloc ((maxVariable+1)*sizeof(int), "permute");
	result->unPermute = (int *) checkMalloc ((maxVariable+1)*sizeof(int), "unPermute");
						...
						int *globalUnMapVariable, int *used, int *isSingleton, int *isUnitClause, ListPtr subProblems)
{
	...
	neighbours = (SetPtr *) checkMalloc ( (1+actualVariables)*sizeof (SetPtr), "neighbours");
		...
		for (v=0;v<clause->numOfVariables;v++)
		{
			...
			if (used[v1] && !isSingleton[v1] && !isUnitClause[v1])
			{
				for (v2=v;v2<clause->numOfVariables;v2++)
				{
					...
					{
						addToSet (neighbours[globalMapVariable[v1]],(void *) globalMapVariable[v3]);
						addToSet (neighbours[globalMapVariable[v3]],(void *) globalMapVariable[v1]);
					}
				}
			}
		}
...
*/
		...
		sprintf (message, "Sub-problem with %d variables", subProblem->proposition->maxVariable);
...

{
...
#ifdef PROOF
	openProofFile (proposition, addExtension (argv[1],"proof"));
#endif
	...
	sprintf (message, "Partial assignments = %d, improvements = %d", tries, improvements);
	dimacsComment (message);
...
#ifdef PROOF
	if (assignment == NULL)
		closeProofFile(getPropositionSize(proposition));
	else
		dimacsComment ("Satisfiable, so no proof generated");
#endif
	...
	writeDIMACSSolution (proposition, assignment, duration, propositionSize,0); 
	closeDIMACSFiles();
