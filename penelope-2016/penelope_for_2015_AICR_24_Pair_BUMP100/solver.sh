...
if [ ! -f ./$solver ]; then
  echo "Building PeneLoPe"
  make CONF=Release > /dev/null || exit 0
fi

if [ "x$1" = "x" ]; then
  echo "USAGE: solver.sh <solver> <input CNF>"
  exit 1
fi
...
TMP=$TEMPDIR/penelope_$$ #set this to the location of temporary files
...
if [ $X == 0 ]; then
  ...
  #SatElite died, penelope must take care of the rest
