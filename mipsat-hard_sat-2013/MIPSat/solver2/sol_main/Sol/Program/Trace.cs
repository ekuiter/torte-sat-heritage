...
namespace Sol
{
	...
	{
		void TraceDecision(int decision)
		{
			this.decision = CNF.B(decision);
		}

		void TraceAssertion(int assertion)
		{
			this.assertion = CNF.B(assertion);
		}

		void TraceResolvent(int[] resolvent)
		{
			CNF.B(resolvent);
			this.resolvent = resolvent;
		}

		void TraceMapping(int[] mapping)
		{
			CNF.BP(mapping);
			this.mapping = mapping;
		}
		
		void TraceModel(int[] model)
		{
			CNF.B(model);
			this.model = model;
		}
