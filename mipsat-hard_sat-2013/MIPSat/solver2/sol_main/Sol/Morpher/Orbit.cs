...
namespace Sol
{
	...
	{
		public int[] ComputeOrbit(Record[] t)
		{
			return ComputeOrbit(t, t.Length);
		}

		int[] ComputeOrbit(Record[] t, int tc)
		{
			int[] o = new int[tc];
			for (int i = 0; i < tc; ++i)
				o[i] = t[i].u;
		...
		public int FindPoint(Record[] t, int u)
		{
			return FindPoint(t, t.Length, u);
		}

		int FindPoint(Record[] t, int tc, int u)
		{
			for (int i = 0; i < tc; ++i)
				if (t[i].u == u)
					return i;
		...
		void ResetMarks(Record[] t, int tc)
		{
			for (int i = 0; i < tc; ++i)
				is_marked[t[i].u] = false;
		}
	}
}
