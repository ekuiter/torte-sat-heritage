...
namespace Sol
{
	public partial class Solver
	{
		...
		void Learn(int a)
		{
			if (bodies[a].Length > 1)
			{
				if (Math.Pow(to_erase.Size, to_erase_power) > ++to_erase_total)
				{
					to_erase.Sort((x, y) => bodies[x].Length - bodies[y].Length);
					int n = to_erase.Size / 2;
					for (int i = to_erase.Size - 1; i >= 0; --i)
						if (locks[to_erase[i]] == 0)
						{
							int b = to_erase.Pop(i);
							Detach(b);
							DestroyClause(b);
